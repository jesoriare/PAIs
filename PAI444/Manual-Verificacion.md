...
El Dockerfile final ya incorpora varias medidas base de endurecimiento:

- usuario no root con `USER appuser`
- directorio de datos dedicado

Trivy sigue siendo útil para comprobar que no habéis reintroducido malas prácticas al tocar el Dockerfile.
...
Debes comprobar que ya no aparezcan hallazgos por ejecución como `root` ni por ausencia de `HEALTHCHECK`.
Si Trivy muestra findings nuevos, deben estar justificados en el informe final o corregidos antes de comprimir.
...
## 17. Preparación del ZIP final

Antes de comprimir:

1. regenera `reports/` si has cambiado código, pipeline o Dockerfile
2. comprueba que `README.md`, `Informe-PAI4.md` y este manual describen la misma versión
3. incluye solo el contenido de `Nuevo/`
4. genera el ZIP con el formato pedido, por ejemplo `PAI4-ST9.zip`
